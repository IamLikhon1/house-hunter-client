...
const SingleCard = ({ item }) => {
    const { name, address, bathrooms, bedrooms, rent, city, number, description } = item;
    console.log(item);
    return (
        <div>
            <div className="border-2 pt-2 pb-5 px-5 rounded-md cursor-pointer">
                <span className="bg-[#F7A582] px-20 py-3 text-white font-bold text-xl relative right-5 rounded-l-md">${rent}</span>
                ...
                <div>
                    ...
                    <p className="text-lg font-semibold my-2">Description: {description.slice(0,100)}. . .</p>

                </div>
                {/* btn */}
                <button className="w-full lg:w-[100%] py-4 px-3 font-semibold border-2 bg-[#07332F] text-white rounded-lg  mt-3 cursor-pointer">Book Appointment</button>
            </div>
        </div>
    );
};
